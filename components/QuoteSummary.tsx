...
interface QuoteSummaryProps {
  ...
  rows: SummaryRow[];
  /** Formatted range from the pricing engine — never computed here. */
  priceMin?: string;
  priceMax?: string;
  /** Manual-review categories show a notice instead of a range. */
  manualNotice?: string;
...
export default function QuoteSummary({
  ...
  const trust = manualNotice ? SUMMARY_TRUST_MANUAL : SUMMARY_TRUST;
        ...
        {manualNotice && (
          <div className="mt-5 rounded-xl bg-teal-50/70 border border-teal-500/20 p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11.5px] font-medium text-slate-700">Individuelle Offerte</span>
            </div>
            <p className="mt-1.5 text-[12px] text-slate-600 leading-relaxed">{manualNotice}</p>
          </div>
        )}
